...
void examine::install_reader(reader *r) {
    readers.push_back(r);
    if (!::initializing)
        r->bind();
}

void examine::install_thread(reader *r) {
    #ifdef _WIN32
...
void examine::install_timer(timer *r) {
    timers.push_back(r);
}

#ifdef _WIN32
    void examine::__lock() { WaitForSingleObject(mutex,INFINITE); }
    void examine::__unlock() { ReleaseMutex(mutex); }
#else
    void examine::__lock() { pthread_mutex_lock(&mutex); }
    void examine::__unlock() { pthread_mutex_unlock(&mutex); }
#endif
...
public:
    virtual bool generate(examine& obj)=0;
    virtual void execute(examine& obj)=0;
    virtual ~gen(){}
...
int examine::___ivy_choose(int rng,const char *name,int id) {
        std::ostringstream ss;
        ss << name << ':' << id;;
...
bool examine::selector__execute_r1(){
    bool y;
...
void examine::__init(){
    {
        ...
        }
        {
            ...
            protocol__r_s2 = (40 & 1023);
            protocol__r1_executions = (0 & 1023);
            protocol__idle = (0 & 1);
        }
    }
}
void examine::ext__protocol__update_r1(){
    {
        ivy_assume((protocol__idle == (0 & 1)), "examine.ivy: line 166");
        {
            bool loc__0;
    loc__0 = (bool)___ivy_choose(0,"loc:0",17);
            {
                ___ivy_stack.push_back(55);
                loc__0 = enabled_checker__is_enabled_r1(protocol__r_s1);
                ___ivy_stack.pop_back();
                ivy_assume(loc__0, "examine.ivy: line 167");
            }
        }
        {
            bool loc__0;
    loc__0 = (bool)___ivy_choose(0,"loc:0",18);
            {
                ___ivy_stack.push_back(56);
                loc__0 = selector__execute_r1();
                ___ivy_stack.pop_back();
                if(loc__0){
                    {
                        ___ivy_stack.push_back(57);
                        inspector__check_guard_r1(protocol__r_s1);
                        ___ivy_stack.pop_back();
                        ___ivy_stack.push_back(58);
                        protocol__r_s1 = updater__decr(protocol__r_s1);
                        ___ivy_stack.pop_back();
                        ___ivy_stack.push_back(59);
                        protocol__r_s1 = updater__incr(protocol__r_s1);
                        ___ivy_stack.pop_back();
                        ___ivy_stack.push_back(60);
                        protocol__r_s2 = updater__incr(protocol__r_s2);
                        ___ivy_stack.pop_back();
                        if((((80 & 1023) < protocol__r_s2) || (protocol__r_s2 == (80 & 1023)))){
                            ___ivy_stack.push_back(61);
...
unsigned examine::updater__incr(unsigned x){
    unsigned y;
    y = (unsigned)___ivy_choose(0,"fml:y",0);
    y = ((x + (1 & 1023)) & 1023);
    return y;
}
void examine::goal__achieved(unsigned v){
    __ivy_out  << "< goal.achieved" << "(" << v << ")" << std::endl;
    {
        ivy_assert((((80 & 1023) < v) || (v == (80 & 1023))), "examine.ivy: line 25");
        protocol__idle = (1 & 1);
        ___ivy_stack.push_back(54);
        imp__goal__achieved(v);
        ___ivy_stack.pop_back();
    }
}
unsigned examine::updater__decr(unsigned x){
    unsigned y;
    y = (unsigned)___ivy_choose(0,"fml:y",0);
    y = ((x - (1 & 1023)) & 1023);
    return y;
}
void examine::imp__goal__achieved(unsigned v){
    {
    }
}
void examine::imp__inspector__check_guard_r1(unsigned reactant1){
    {
    }
}
void examine::inspector__check_guard_r1(unsigned reactant1){
    __ivy_out  << "< inspector.check_guard_r1" << "(" << reactant1 << ")" << std::endl;
    {
        ivy_assert((((1 & 1023) < reactant1) || (reactant1 == (1 & 1023))), "examine.ivy: line 48");
        ___ivy_stack.push_back(62);
        imp__inspector__check_guard_r1(reactant1);
        ___ivy_stack.pop_back();
    }
}
bool examine::enabled_checker__is_enabled_r1(unsigned reactant1){
    bool y;
    y = (bool)___ivy_choose(0,"fml:y",0);
...
void examine::ext__protocol__idling(){
    {
        ivy_assume((protocol__idle == (1 & 1)), "examine.ivy: line 172");
    }
}
void examine::__tick(int __timeout){
}
examine::examine(){
#ifdef _WIN32
...
public:
    init_gen();
    bool generate(examine&);
    void execute(examine&){}
};
init_gen::init_gen(){
...
bool init_gen::generate(examine& obj) {
    alits.clear();
...
bool ext__protocol__update_r1_gen::generate(examine& obj) {
    push();
...
void ext__protocol__update_r1_gen::execute(examine& obj){
...
bool ext__protocol__idling_gen::generate(examine& obj) {
    push();
    slvr.add(__to_solver(*this,apply("protocol.idle"),obj.protocol__idle));
    alits.clear();
...
void ext__protocol__idling_gen::execute(examine& obj){
...
int ask_ret(long long bound) {
    ...
    class examine_repl : public examine {
...
public:
    ...
    cmd_reader(examine_repl &_ivy) : ivy(_ivy) {
        lineno = 1;
        if (isatty(fdes()))
